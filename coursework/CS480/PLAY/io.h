 ...
 */
void io_remove_type(enum Consumers,RequestType, int inRequestQueue[], int consumed[]);
/*
 * void io_production_report(int produced[], int *consumed[])
 * Show how many requests of each type produced.  Show how many requests consumed 
by
 * each consumer.
 * produced[] - count for each RequestType
 * consumed[] - array of pointers to consumed arrays for each consumer
 *    e.g. consumed[CostAlgoDispatch] points to an array that is indexed by request
type
 *    (consumed[CostAlgoDispatch][HumanDriver] is the number for requesting human 
drivers 
